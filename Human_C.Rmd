...
```
```{r}
C_Cities <- read.csv("Data_Output/distance_to_cities.csv", stringsAsFactors = F) %>% rename(Time_min = Travel_Time_min)

Carbon <- read.csv("Data_Output/Carbon_Per_Plots.csv", stringsAsFactors = F)
Carbon_Cities <- Carbon %>% left_join(C_Cities,by.x = Plot_names) %>% dplyr::select(-EAST,-NORTH) %>% drop_na(Time_min)

lm.CarbonCities <- lm(Sum_CHa~Time_min, data = Carbon_Cities)
anova(lm.CarbonCities)
plot(lm.CarbonCities)

ggplot(data=Carbon_Cities, aes(x=Time_min, y = Sum_CHa, color=District))+ geom_line()+ 
...
ggplot(data = Carbon_Cities, mapping=aes(x=Time_min,y=Sum_CHa, color=District))+geom_line()+
